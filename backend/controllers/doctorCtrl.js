...
const getDoctorbyid = async (req,res)=>{
  try {
    ...
    res.status(200).send({success: true,message: "Sigle Doc Info Fetched",data: doctor});
  } catch (error) {
    console.log(error);
    res.status(500).send({success: false,error,message: "Erro in Single docot info"});
  }
}

module.exports = { getDoctorInfoController, updateProfileController,getDoctorbyid };